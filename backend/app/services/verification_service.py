...
class VerificationService:
    ...
    def verify(self, articles: Iterable[SimilarArticle]) -> VerificationResult:
        ...
        similarity_component = weighted_average
        source_factor= self._calculate_source_factor(len(unique_sources))
        ...
        # Make the raw score a balanced combination rather than strict multiplication
        # Base score is primarily driven by how similar the articles are and their credibility
        base_score = similarity_component
        
        # Boost based on the number of unique sources confirming it (up to 30% boost)
        source_boost = 1.0 + (source_factor * 0.3)
        
        # Boost based on diversity of sources (up to 20% boost)
        diversity_boost = 1.0 + (diversity_factor * 0.2)
        
        # Recency should not penalize the truthfulness of a historical news. 
        raw_score = base_score * source_boost * diversity_boost
    ...
    def _calculate_source_factor(self, unique_sources: int) -> float:
        """Return source credibility factor as described in the documentation."""
        if unique_sources <= 0:
            return 0.0

        return _clamp(unique_sources / float(self._min_sources),0.0, 1.0    )
